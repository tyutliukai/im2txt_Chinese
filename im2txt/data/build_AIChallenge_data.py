...
The AIChallenge images are expected to reside in JPEG files located in the following
directory structure:

  train_image_dir/0a00a9aa4e31fb51611afb11752ab8b0732894ee.jpg
  ...
  train_image_dir/0a0a410ce70196cb2eedfbf8e32ab0aa51c44e10.jpg

and


  val_image_dir/00a00cb71b4a3fa91be53e4ee815c70741712ce5.jpg
  ...
  val_image_dir/0a1aa566146fc4f2c7c14195edfa9031ce707012.jpg
...
of 512, 64 TFRecord files, respectively:

  output_dir/train-00000-of-00512
  output_dir/train-00001-of-00512
  ...
  output_dir/train-00511-of-00512

and

  output_dir/val-00000-of-00064
  ...
  output_dir/val-00063-of-00064


Each TFRecord file contains ~2050 records. Each record within the TFRecord file
...
The SequenceExample proto contains the following fields:
  ...
  context:
    image/image_id: integer AIChallenge image identifier
...
NOTE: This script will consume around 150GB of disk space because each image
...
tf.flags.DEFINE_string("word_counts_output_file",
                       "/Users/TimoYoung/workspace/PyCharm/im2txt/word_count.txt",
...
def _load_and_process_metadata(captions_file, image_dir):
    ...
    id_captions = [(x["image_id"], x["caption"]) for x in caption_data]
    print("Loaded caption metadata for %d images from %s" % (len(id_captions), captions_file))
    ...
    image_metadata = []
    num_captions = 0
    for image_id, captions in id_captions:
        filename = os.path.join(image_dir, image_id)
        captions = [_process_caption(c) for c in captions]
        image_metadata.append(ImageMetadata(image_id, filename, captions))
        num_captions += len(captions)
    print("Finished processing %d captions for %d images in %s" %
          (num_captions, len(id_captions), captions_file))
...
def main(unused_argv):
    ...
    #                                                 FLAGS.val_image_dir)
    # AI_Challenger_test_dataset = _load_and_process_metadata(FLAGS.test_captions_file,
    ...
    train_cutoff = int(0.8 * len(AI_Challenger_train_dataset))
    val_cutoff = int(0.90 * len(AI_Challenger_train_dataset))

    train_dataset = AI_Challenger_train_dataset[:train_cutoff]
    val_dataset = AI_Challenger_train_dataset[train_cutoff:val_cutoff]
    test_dataset = AI_Challenger_train_dataset[val_cutoff:]
